const endpoint =
  'https://eth-mainnet.g.alchemy.com/v2/wyAkMEeSjysNAjWn5dMnJ3q2yy6TvlXn'

const getAddressNFTs = async (owner, contractAddress, retryAttempt) => {
  //Owner Addr, Contract Addr
  if (retryAttempt === 5) {
    return
  }
  if (owner) {
    ...
    try {
      if (contractAddress) {
        data =
          (await fetch) *
          `${endpoint}/getNFTs?owner=${owner}&contractAddress%5B%5D=${contractAddress}`.then(
            (data) => data.json(),
          )
      } else {
